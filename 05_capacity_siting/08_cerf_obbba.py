"""
OBBBA-REGIME CERF siting  (third policy regime: no-policy / IRA-policy / OBBBA).
Identical to 07_cerf_policy.py EXCEPT two OBBBA changes:
  (A) capacity input = /data/gcam_usa/gcam_capacity_obbba.csv (post-2027 wind/solar additions
      cut for the 45Y-PTC/48E-ITC termination: wind retention 0.47, solar 0.80).
  (B) siting incentive = STATE-ONLY RPS/CES pull: the federal tax-credit share of the combined
      clean-siting NLC bonus is removed by halving alpha (0.08 -> 0.04). Local ordinance/ban/
      setback + prime-farmland exclusions are UNCHANGED (state/local, unaffected by OBBBA).
...
Outputs: /data/cerf_out/fleet_obbba_{scen}.csv (+ policy_diag_{scen}.csv, map_obbba_{scen}.png,
         SUMMARY_obbba_fleets.csv)
"""
...
GCAM_CSV="/data/gcam_usa/gcam_capacity_obbba.csv"
...
RPS_ALPHA=float(os.environ.get("RPS_ALPHA","0.04"))  # OBBBA: state-only half of the IRA 0.08 combined federal+state clean-siting pull
...
def reconcile_fleet_to_gcam(fl,piv):
    """FLEET-FIX: force the realized 2050 fleet to EQUAL the GCAM(OBBBA)-2050 capacity target per
    ...
    shortfall of >= one unit is genuine unmet demand (here, capacity STRANDED by the policy
    exclusion mask) and is left untouched. Retirement is recorded via retirement_year=2050 ->
...
def run_scenario(climate,ssp):
    ...
    #     fleet, so additions are driven by the ABSOLUTE GCAM(OBBBA) path (reconciled to the seed),
    #     not by gross period-to-period diffs that ignore GCAM declines. ---
    ...
    for Y in YEARS:
        ...
        nlc=ic-nov-rps_bonus                            # <<< OBBBA: state-only RPS/CES pull (federal PTC/ITC dropped: alpha 0.08->0.04)
        # FLEET-FIX: net additions to raise OPERATING capacity toward the ABSOLUTE GCAM(OBBBA) path.
    ...
    fl.to_csv(f"{OUT}/fleet_obbba_{scen}{os.environ.get('OUTTAG','')}.csv",index=False)
    ...
    print(f"  [{scen}] wrote fleet_obbba_{scen}.csv n={len(fl)} "
...
def summarize(scen,fl,cf_alb,ssp,strand_wind,strand_solar):
    ...
    plt.tight_layout(); plt.savefig(f"{OUT}/map_obbba_{scen}.png",dpi=100); plt.close()
    return rec
...
if __name__=="__main__":
    ...
    S.to_csv(f"{OUT}/SUMMARY_obbba_fleets{tag}.csv",index=False)
