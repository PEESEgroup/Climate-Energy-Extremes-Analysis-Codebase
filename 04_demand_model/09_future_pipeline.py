...
SCEN=sys.argv[1]
SRC=f"/data/tgw_extract/{SCEN}"; FDIR=f"/data/tell_forcing/{SCEN}"; os.makedirs(FDIR,exist_ok=True)
...
YEARS=list(range(2030,2051)); t0=time.time(); log=lambda s: print(f"[{SCEN} {time.time()-t0:6.0f}s] {s}",flush=True)
...
files=sorted(glob.glob(SRC+"/*hourly*.npz")); log(f"{len(files)} future npz")
used=0
