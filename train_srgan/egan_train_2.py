...
channel_group = 2  # 👈 Customize the channel group to use
year = ['2019','2020']
# ==== Global Paths ====
dir_input = Path('your_directory/upstream_low')
dir_output = Path('your_directory/dnstream/input')
checkpoint_dir = Path(f'your_directory/result/weather{channel_group}')
...
def train_srgan_ddp(
    ...
    d_optimizer = optim.Adam(D.parameters(), lr=5e-6, betas=(0.9, 0.999))
    ...
    for epoch in range(n_epoch):
        ...
        for batch in train_loader_adv:
            ...
            g_loss = pixel_loss + 5e-2 * adv_loss + 1e-2 * shadow_loss
            ...
            if rank == 0:
                print(f"[Epoch {epoch+1}/{n_epoch}] [Rank {rank}] {processed/total*3*100:.1f}%--d_loss: {d_loss:.6f}, pixel: {pixel_loss:.6f}, adv: {adv_loss:.6f}, grad: {grad_loss:.6f}, g_loss: {g_loss:.6f}",end="\r")
...
def evaluate_adversarial(G, D, val_loader, device, rank=0, amp=False):
    ...
    total_val_loss = avg_mse + 5e-2 * avg_adv + 1e-2 * avg_shadow
...
def ddp_main(rank, world_size, channel_group):
    ...
    train_srgan_ddp(G, D, rank=rank, world_size=world_size, channel_group=channel_group)
    cleanup()
